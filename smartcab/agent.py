...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=True, epsilon=1., alpha=0.25):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            if self.eps_function == 'linear_1000':
                self.epsilon = 1 - .001 * self.trial_number
            if self.eps_function == 'exponential':
                a = .95
                self.epsilon = math.pow(a, self.trial_number)
            if self.eps_function == 'power_minus_two':
                if self.trial_number == 0:
                    self.epsilon = 1
                else:
                    self.epsilon = 1 / math.pow(self.trial_number, 2)
            if self.eps_function == 'e_power':
                a = .05
    ...
    def get_maxQ(self, state):
        ...
        max_action, maxQ = max(self.Q[state].iteritems(), key=lambda x: x[1])
